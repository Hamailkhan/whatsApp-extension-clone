...
document.addEventListener('whatsappContentToWhatsappJs', async function (e, data) {
    if (e.detail.action === "getChats") {
        try {
            const chats = await window.WWebJS.getChats(); // Fetch all chats
            console.log("chats", chats);
            document.dispatchEvent(new CustomEvent("whatsappJsToContent", {
                detail: { chats }
            }));
        } catch (error) {
